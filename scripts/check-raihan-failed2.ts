...
async function main() {
  console.log("subTopic|studentId|hasQuiz|rawLen|procLen|materialId");
  for (const sub of FAILED) {
    const rows: any[] = await prisma.$queryRawUnsafe(
      `SELECT m.id, m."subTopic", m.subject, s."studentId", q.id as quiz_id,
    ...
    if (rows.length === 0) {
      console.log(`${sub}|NONE|null|null|null|null`);
    } else {
      for (const r of rows) {
        console.log([sub, r.studentId, r.quiz_id ? "HAS_QUIZ" : "NO_QUIZ", r.raw_len, r.proc_len, r.id].join("|"));
      }
    }
  }
