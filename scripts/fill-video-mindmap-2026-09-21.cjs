#!/usr/bin/env node
/**
 * Fill videoUrl + metadata.mindmap for the 21 new Moodle materials
 * (metadata.addedBy = 'moodle-2026-09-21').
 *
 * Serial, idempotent, additive. One LLM call per material for the mindmap;
 * videoUrl is the same YouTube search-query form the existing batch-generate
 * used for these students (no API key needed, always valid).
 *
...
// YouTube search-query link — same shape the existing rows use.
...
// Extract the first balanced JSON object/array from a possibly-fenced blob.
...
// Normalize whatever the model returned into {id,label,children} single root.
function normalizeMindmap(obj, rootLabel) {
  let root = Array.isArray(obj) ? obj[0] : obj;
  ...
  const walk = (node, prefix) => {
    ...
    out.children = kids.map((c, i) => {
      n++;
      return walk(c, `${prefix || "n"}${i + 1}`);
    });
    return out;
  };
  const tree = walk(root, "root");
  // sanity: need at least a root + 2 branches
...
(async () => {
  const rows = (await pool.query(
    `SELECT m.id, m.subject, m.topic, m."subTopic",
            LEFT(COALESCE(m.metadata->>'slide', m."rawContent", ''), 1800) AS slide,
            (m."videoUrl" IS NULL OR m."videoUrl" = '') AS need_video,
            (m.metadata->'mindmap' IS NULL) AS need_mindmap
       FROM "Material" m
      WHERE m.metadata->>'addedBy' = $1
      ORDER BY m.subject, m.topic`, [TAG])).rows;

  console.log(`Found ${rows.length} tagged materials.\n`);
  let vDone = 0, mDone = 0, mFail = 0;

  for (let i = 0; i < rows.length; i++) {
    const r = rows[i];
    const tag = `[${i + 1}/${rows.length}] ${r.subject} → ${r.topic}${r.subTopic ? " → " + r.subTopic : ""}`;
    console.log(tag);
    const sets = [], vals = [];
    let p = 1;

    // ---- videoUrl (deterministic, no LLM) ----
    if (r.need_video) {
      const url = videoSearchUrl(r.subject, r.topic, r.subTopic);
      sets.push(`"videoUrl"=$${p++}`); vals.push(url);
      vDone++;
      console.log("  🎬 videoUrl set");
    }

    // ---- mindmap (LLM) ----
    if (r.need_mindmap) {
      const ctx = r.slide && r.slide.length > 50 ? r.slide.slice(0, 1600) : `${r.topic} - ${r.subTopic || ""}`;
      const prompt =
...
Aturan: root label = judul topik singkat; 3-5 cabang utama; tiap cabang 1-3 sub-cabang ringkas (max ~8 kata). WAJIB sertakan field "children":[] di setiap node (termasuk daun). Jangan lebih dari 4 cabang utama dan 3 anak per cabang.`;
      ...
      if (txt) {
        ...
        const tree = parsed ? normalizeMindmap(parsed, r.subTopic || r.topic) : null;
        if (tree) {
          sets.push(`metadata=jsonb_set(COALESCE(metadata,'{}'::jsonb),'{mindmap}',$${p++}::jsonb,true)`);
          vals.push(JSON.stringify(tree));
          mDone++;
          console.log(`  🗺️ mindmap set (${tree.children.length} cabang)`);
        } else {
          mFail++; console.log("  ❌ mindmap: JSON tak bisa dipakai");
        }
      } else {
        mFail++; console.log("  ❌ mindmap: LLM gagal");
      }
    }
