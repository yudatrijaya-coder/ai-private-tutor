...
function StudentHeader({ studentName, streak, stars }: { studentName: string; streak: number; stars: number }) {
...
function StudentLayoutInner({ children, streak, stars }: { children: React.ReactNode; streak: number; stars: number }) {
  const pathname = usePathname();
  ...
  return (
    ...
    >
      <StudentHeader studentName={studentName} streak={streak} stars={stars} />
...
export default function StudentLayoutClient({
  ...
  return (
    <Suspense fallback={<div className="min-h-screen" style={{ backgroundColor: "var(--st-bg)" }} />}>
      <StudentLayoutInner streak={streak} stars={stars}>
        <StudyTracker />
        {children}
      </StudentLayoutInner>
    </Suspense>
